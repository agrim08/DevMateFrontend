...
import { MessageSquare, Users, MapPin, ShieldCheck } from "lucide-react"
...
const ConnectionCard = ({ connection, idx }) => {
  ...
  const { _id, photoUrl, skills, firstName, lastName, bio, gender, membershipType } = connection
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 mb-8">
          ...
          {!isExpanded && skillsArray.length > 3 && (
          <button 
              onClick={() => setIsExpanded(true)}
              className="text-[10px] text-primary hover:underline flex items-center px-1 font-bold cursor-pointer"
          >
          ...
          {isExpanded && skillsArray.length > 3 && (
            <button 
                ...
                className="text-[10px] text-muted-foreground hover:underline flex items-center px-1 font-bold cursor-pointer"
            >
                Show less
            </button>
          )}
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {connectionData.map((connection, idx) => (
            <ConnectionCard key={connection._id} connection={connection} idx={idx} />
          ))}
        </div>
      </div>
    </div>
