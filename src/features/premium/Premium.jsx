...
const Premium = () => {
  ...
  const handleMembership = async (membershipType) => {
    ...
    setIsLoading(true)
    try {
      const response = await axiosInstance.post("/payment/create-order", { membershipType })
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            const verificationRes = await axiosInstance.post("/payment/verify", response)
          ...
          {plans.map((plan, idx) => (
            ...
            >
              ...
              <Button
                ...
                loading={isLoading}
                disabled={(user?.membershipType === "diamond" && plan.name === "Emerald")}
